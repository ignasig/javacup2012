...
import org.javahispano.javacup.tacticas.tacticas_aceptadas.mansporting.MSGSituacionPartidoContext.AlturaComparator;
import org.javahispano.javacup.tacticas.tacticas_aceptadas.mansporting.thread.JugadoresBalonData;
import org.javahispano.javacup.tacticas.tacticas_aceptadas.mansporting.thread.PaseData;
...
public abstract class MSGAbstractJugadorPropio extends MSGAbstractJugadorDetalle implements MSGJugadorPropio {
    ...
    protected Command pasa() {
        ...
        if (pase == null) {
            ...
            double fuerza = pase.getTrayectoriaPunto().getTrayectoria().getVelocidad() / Constants.getVelocidadRemate(getEstadisticas().getRemate());
    ...
    protected CommandHitBall tira(Punto destino) {
        TrayectoriaPunto candidato = TrayectoriaManager.getInstance().getDisparosByDistancia(
                TrigonometriaUtils.getDistancia(context.getBalon().getPosicion().getPuntoXY(), destino));
    ...
    protected final PaseData getMejorPase() {
        ...
        List<Comparator<PaseData>> comparators = Arrays.<Comparator<PaseData>>asList(new AlturaComparator());
    ...
    private Punto3D desmarqueCercano(Punto3D posicionBalon) {
        ...
        if (!getAlineacion().getPosicion().equals(posicionBalon.getPuntoXY())) {
            Segmento segmento = new Segmento(getAlineacion().getPosicion(), posicionBalon.getPuntoXY());
